...
- The Metropolis-Hastings algorithm is useful for drawing samples from Bayesian posterior distributions, especially when the normalizing constant is unknown or difficult to compute . The Gibbs sampler can be viewed as a special case of Metropolis-Hastings (as well will soon see).