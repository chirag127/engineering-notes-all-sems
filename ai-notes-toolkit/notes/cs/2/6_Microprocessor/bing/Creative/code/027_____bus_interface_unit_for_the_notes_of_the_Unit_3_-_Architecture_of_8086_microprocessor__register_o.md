...
- The BIU uses three different buses to transfer data and instructions between the microprocessor and other components in a computer system. These buses are:
  - Address bus: The address bus is used to send the memory address of the instruction or data being read or written. It is a 20-bit unidirectional bus, which can address up to 1 MB of memory.
  ...
  - Control bus: The control bus is used to send the control signals that determine the direction and type of data transfer. It consists of various signals such as read, write, interrupt, etc.