...
In mathematical terms, a distance measure is represented by a two-argument function d: S(H) x S(H) → R. The basic properties associated with a distance d are that it is a metric, i.e., it satisfies positivity: d(ρ, σ) ≥ 0 with equality if and only if ρ = σ.
