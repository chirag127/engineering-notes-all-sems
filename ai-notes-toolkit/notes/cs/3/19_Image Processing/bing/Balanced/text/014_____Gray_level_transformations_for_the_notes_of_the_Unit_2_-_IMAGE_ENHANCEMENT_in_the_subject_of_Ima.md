...
- Some common types of gray level transformations are:
  ...
  - Logarithmic transformation: s = T(r) = c log(1 + r), where c is a constant. This transformation compresses the dynamic range of the image, making dark regions brighter and bright regions darker. It is useful for enhancing details in low-light images.
  ...
  - Piecewise-linear transformation: s = T(r) = a r + b, where a and b are constants that vary for different ranges of r. This transformation allows for more flexibility and control over the image enhancement, as different linear functions can be applied to different segments of the input gray level range. Some examples of piecewise-linear transformations are:

    - Contrast stretching: s = T(r) = a r + b, where a > 1 and b < 0. This transformation increases the contrast of the image by expanding the output gray level range.
    ...
    - Gray level slicing: s = T(r) = A, if r is in a specified range [a, b]; s = T(r) = r, otherwise, where A is a constant. This transformation highlights a specific range of gray levels in the image by assigning them a high value, while preserving the rest of the image.
    - Bit-plane slicing: s = T(r) = 2^k, if the k-th bit of r is 1; s = T(r) = 0, otherwise, where k is an integer between 0 and L - 1. This transformation extracts a single bit-plane from the image, where each pixel is either 0 or 2^k depending on the value of the k-th bit. This can be useful for analyzing the contribution of different bit-planes to the image quality or information content.