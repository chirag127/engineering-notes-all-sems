...
- In the validation phase, the transaction is checked for serializability using certain validation rules and time-stamps. If the transaction passes the validation, it can proceed to the write phase. Otherwise, it is aborted and restarted  .
- In the write phase, the transaction writes the updated values from the local copies to the database. No other transaction can access the database during this phase  .
- The validation based protocol ensures serializability and avoids deadlock, but it may cause more aborts and restarts than other protocols  .