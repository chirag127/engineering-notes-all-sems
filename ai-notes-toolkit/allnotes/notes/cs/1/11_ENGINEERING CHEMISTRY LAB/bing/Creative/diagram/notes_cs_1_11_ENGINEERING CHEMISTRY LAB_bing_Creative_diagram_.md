...
```mermaid
graph LR
A(Urea + Formaldehyde) -->|pH 6-11, 80°C, acid| B(MMU + Water)
B -->|pH 7-9, 70-90°C, formaldehyde| C(DMU + Water)
C -->|pH 4-5, 90-100°C, formaldehyde + urea| D(UF resin + Water)
```
...
```mermaid
graph LR
A[Urea] -->|Formaldehyde| B[Monomethylolurea]
B -->|Formaldehyde| C[Dimethylolurea]
C -->|Acid catalyst| D[Prepolymer]
D -->|Heat, pressure, or hardener| E[UF resin]
```
...
```mermaid
graph LR
A(Phenol + Formaldehyde) --> B(Reaction vessel)
B --> C(Catalyst)
C --> D(Water + Formaldehyde removal)
D --> E(Curing)
E --> F(PF resin)
```
