...
- There are three types of elementary transformations for matrices :
  - Row transformations: These are operations performed on the rows of a matrix, such as interchanging two rows, multiplying a row by a nonzero scalar, or adding a multiple of one row to another row.
  - Column transformations: These are operations performed on the columns of a matrix, such as interchanging two columns, multiplying a column by a nonzero scalar, or adding a multiple of one column to another column.
  - Row-column transformations: These are operations performed on both the rows and columns of a matrix, such as multiplying a row by a scalar and then adding it to a column, or vice versa.
- Elementary transformations can be represented by elementary matrices, which are square matrices that are obtained by applying one elementary transformation to the identity matrix .
...
- The determinant of an elementary matrix is equal to the factor by which the determinant of the original matrix is changed by the corresponding elementary transformation.
- The inverse of an elementary matrix is another elementary matrix that reverses the effect of the original elementary transformation.
...
- The rank of a matrix is also equal to the order of the highest ordered non-zero minor .
- A minor of a matrix is the determinant of a submatrix obtained by deleting some rows and columns from it.
...
- The rank of a matrix can be found by using various methods, such as row reduction, Gaussian elimination, or finding the non-zero eigenvalues.
...
- For example, if A = [ i 1 + i 2 − 3 i − 1 + i 2 i 1 − 2 − 3 i − 1 0], then A* = [ −i − 1 − i − 2 + 3 i 1 − i − 2 − i − 1 + 2 + 3 i 1 0] and −A = [ −i − 1 − i − 2 + 3 i 1 − i − 2 − i − 1 + 2 + 3 i 1 0], so A is a skew-Hermitian matrix.
...
- The characteristics of the skew-Hermitian matrix are as follows    :
  ...
  - Any skew-Hermitian matrix is diagonalizable. Its eigenvalues are either purely imaginary or zeros.
  - The diagonal elements of a skew-Hermitian matrix are always equal to zero.
  - The sum of two skew-Hermitian matrices is also a skew-Hermitian matrix.
  - The product of two skew-Hermitian matrices is a skew-Hermitian matrix if and only if they commute, that is, AB = BA.
  - The inverse of a skew-Hermitian matrix is also a skew-Hermitian matrix, if it exists.
  - The conjugate of a skew-Hermitian matrix is also a skew-Hermitian matrix.
  - The determinant of a skew-Hermitian matrix is a real number.
...
- The unitary matrix is a non-singular matrix, meaning that its determinant is not zero.
- The unitary matrix is an invertible matrix, meaning that it has a unique inverse.
- The product of two unitary matrices is a unitary matrix, meaning that unitary matrices form a group under matrix multiplication.
- The inverse of a unitary matrix is another unitary matrix, meaning that unitary matrices are closed under inversion.
- A matrix is unitary if and only if its transpose is unitary, meaning that unitary matrices are symmetric with respect to conjugation and transposition.
- A matrix is unitary if its rows are orthonormal and its columns are orthonormal, meaning that the dot product of any two distinct rows or columns is zero and the norm of any row or column is one.

Some examples of unitary matrices are:

- The identity matrix I is a unitary matrix, since I^H I = I I^H = I.
- The matrix A = [2^(-1/2) 2^(-1/2) 0; -2^(-1/2)i 2^(-1/2)i 0; 0 0 i] is a unitary matrix, since A^H A = A A^H = I and its rows and columns are orthonormal.
- The matrix B = [1/sqrt(2) 1/sqrt(2); -1/sqrt(2) 1/sqrt(2)] is a unitary matrix, since B^H B = B B^H = I and its rows and columns are orthonormal. This matrix is also an example of a real unitary matrix, which is the same as an orthogonal matrix.
...
Differential calculus is the branch of mathematics that studies the rates of change of functions and their properties. It is one of the two main branches of calculus, along with integral calculus.

Some of the topics covered in this unit are:

- **Limits and continuity**: A limit is the value that a function approaches as its input gets closer to a certain point. Continuity is the property of a function that means it has no breaks or gaps in its graph. A function is continuous at a point if its limit at that point equals its value at that point.
- **Derivatives and rules of differentiation**: A derivative is the rate of change of a function at a given point. It measures how sensitive the function is to small changes in its input. The rules of differentiation are formulas that allow us to find the derivatives of various types of functions, such as polynomials, trigonometric, exponential, logarithmic, etc.
- **Applications of derivatives**: Derivatives have many applications in various fields of science, engineering, economics, etc. Some of the common applications are finding the slope and equation of a tangent line, finding the maximum and minimum values of a function, finding the rate of change of a quantity, finding the elasticity of demand, etc.
- **Higher-order derivatives**: Higher-order derivatives are the derivatives of the derivatives of a function. They measure how the rate of change of a function changes as the input changes. For example, the second derivative of a function is the rate of change of its first derivative, and the third derivative is the rate of change of its second derivative, and so on. Higher-order derivatives can be used to study the concavity, inflection points, and curvature of a function.
...
Curve tracing is the process of sketching the graph of a curve given its equation in cartesian, polar or parametric form. It involves finding the domain, range, symmetry, intercepts, asymptotes, intervals of increase and decrease, local extrema, points of inflection, concavity and curvature of the curve.

Some general steps for curve tracing are:

- Identify the type of curve (cartesian, polar or parametric) and its degree (linear, quadratic, cubic, etc.).
- Find the domain and range of the curve by solving for the values of x and y that make the equation valid.
- Find the symmetry of the curve by checking if the equation is unchanged when x or y is replaced by -x or -y, or when x and y are interchanged.
- Find the x-intercepts and y-intercepts of the curve by setting y or x equal to zero and solving for the other variable.
- Find the vertical and horizontal asymptotes of the curve by finding the limits of y as x approaches infinity or negative infinity, or vice versa.
- Find the intervals of increase and decrease of the curve by finding the first derivative of y with respect to x and checking its sign.
- Find the local maxima and minima of the curve by finding the critical points where the first derivative is zero or undefined, and applying the first or second derivative test.
- Find the points of inflection of the curve by finding the second derivative of y with respect to x and checking where it changes sign.
- Find the concavity and curvature of the curve by finding the sign of the second derivative and the value of the radius of curvature.
- Sketch the curve using the above information and adding any other relevant features such as cusps, loops, branches, etc.
...
- For example, if f(x,y) = xy + x^2y, then the partial derivative of f with respect to x is f_x(x,y) = y + 2xy, and the partial derivative of f with respect to y is f_y(x,y) = x + x^2.
- To find the partial derivative of a function, we treat the other variables as constants and use the usual rules of differentiation, such as the power rule, the product rule, the quotient rule, and the chain rule .
- The notation for partial derivatives can vary, but some common symbols are:

  - f_x(x,y) or \frac{\partial f}{\partial x}(x,y) for the partial derivative of f with respect to x
  - f_y(x,y) or \frac{\partial f}{\partial y}(x,y) for the partial derivative of f with respect to y
  - f_{xx}(x,y) or \frac{\partial^2 f}{\partial x^2}(x,y) for the second partial derivative of f with respect to x
  - f_{yy}(x,y) or \frac{\partial^2 f}{\partial y^2}(x,y) for the second partial derivative of f with respect to y
  - f_{xy}(x,y) or \frac{\partial^2 f}{\partial x \partial y}(x,y) for the mixed partial derivative of f with respect to x and y
  - f_{yx}(x,y) or \frac{\partial^2 f}{\partial y \partial x}(x,y) for the mixed partial derivative of f with respect to y and x

- Partial derivatives are useful for studying the rate of change of a function along different directions, such as the slope of a surface or the sensitivity of a physical quantity .
- Partial derivatives also play an important role in multivariable calculus, such as finding the gradient, the divergence, the curl, and the Laplacian of a function.
...
- Change of variables is a technique that allows us to transform a given function or equation into a simpler or more convenient form by introducing new variables that are related to the original ones.
- Change of variables is useful in differential calculus for solving different types of problems, such as:
  - Finding derivatives of composite functions using the chain rule.
  - Reducing nonlinear differential equations to linear ones by applying suitable substitutions.
  - Evaluating integrals by reversing the chain rule, also known as integration by substitution.
- Change of variables can be applied to functions of one or more variables, depending on the context and the problem.
- Change of variables requires us to find the relationship between the new and the old variables, and to express the function or equation in terms of the new variables. This may involve using algebraic manipulation, implicit differentiation, or integration by parts.
- Change of variables also requires us to adjust the limits of integration, if any, according to the new variables. This may involve using inverse functions, graphs, or geometric reasoning.
- Change of variables can simplify the computation and the interpretation of the results, but it also introduces some challenges, such as:
  - Choosing an appropriate substitution that makes the problem easier to solve.
  - Keeping track of the variables and their domains, ranges, and units.
  - Applying the correct formulas and rules for derivatives, integrals, and differential equations.
  - Checking the validity and the accuracy of the solution.
...
- The main objectives of this unit are:
  ...
  - To understand the applications of power series in various fields such as physics, engineering, and mathematics.
...
- The beta function satisfies the recurrence relation $B(x+1,y) = \frac{x}{x+y} B(x,y+1)$ for any positive $x$ and $y$.
- The beta function can be expressed in terms of the hypergeometric function as $B(x,y) = \frac{x}{y} {}_2F_1(1,x;x+y;1)$ for any positive $x$ and $y$.
...
- The gamma function satisfies the functional equation $\Gamma(x+1) = x\Gamma(x)$ for any positive $x$.
...
- The gamma function can be expressed in terms of the incomplete gamma function as $\Gamma(x) = \gamma(x,0) + \Gamma(x,0)$ for any positive $x$, where $\gamma(x,a) = \int_0^a t^{x-1} e^{-t} dt$ and $\Gamma(x,a) = \int_a^\infty t^{x-1} e^{-t} dt$ are the lower and upper incomplete gamma functions, respectively.
...
- The theorem can be interpreted as a conservation law: the net flux of **F** through the boundary of a region is equal to the net source or sink of **F** in the region.
- The theorem can be proved using the divergence theorem in two dimensions, also known as Green's theorem, and applying it to the projection of **V** and **S** onto the coordinate planes.
- The theorem can be used to simplify the calculation of surface integrals, especially when the divergence of **F** is easier to compute than **F** itself. It can also be used to derive other important results in physics and mathematics, such as Gauss's law for electric fields, the continuity equation for fluid flow, and the divergence form of the heat equation .
