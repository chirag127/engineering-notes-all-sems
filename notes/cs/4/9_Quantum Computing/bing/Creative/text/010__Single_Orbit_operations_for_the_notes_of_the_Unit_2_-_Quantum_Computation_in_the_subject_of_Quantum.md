...
- A single qubit operation is a transformation that acts on a single qubit, changing its state in some way.
...
- Some examples of single qubit operations are:
  ...
  - The Pauli-Y operation, which flips the qubit state from |0> to -i|1> and from |1> to i|0>. It is represented by the matrix Y = [[0, -i], [i, 0]] or a rotation of pi radians around the y-axis on the Bloch sphere.
  - The T operation, which adds a phase of pi/4 to the qubit state. It is represented by the matrix T = [[1, 0], [0, e^(i*pi/4)]] or a rotation of pi/4 radians around the z-axis on the Bloch sphere.
  - The S operation, which adds a phase of pi/2 to the qubit state. It is represented by the matrix S = [[1, 0], [0, i]] or a rotation of pi/2 radians around the z-axis on the Bloch sphere.