...
- Curl of a vector field $\vec{F}$ at a point $P$ is denoted by $\nabla \times \vec{F}(P)$ and is defined as the vector whose magnitude is the limit of the net circulation of $\vec{F}$ per unit area as the area shrinks to $P$, and whose direction is perpendicular to the area and follows the right-hand rule.
...
- Physically, curl measures the tendency of the fluid to swirl or rotate around a point. A nonzero curl means that the fluid is spinning or curling around the point, while a zero curl means that the fluid is not spinning or curling, but moving in a straight line or not moving at all. The direction of the curl vector indicates the axis of rotation of the fluid, and the magnitude of the curl vector indicates the strength of the rotation.
