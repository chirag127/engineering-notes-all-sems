...
- There are three types of session beans: stateless, stateful, and singleton .
  - Stateless session beans do not maintain a conversational state with the client and can be pooled and reused by different clients .
...
- There are two types of entity beans: container-managed persistence (CMP) and bean-managed persistence (BMP).
  ...
  - BMP entity beans implement the persistence logic in the bean code, which gives more control and flexibility to the developer.
...
- Message-driven beans are stateless and can be pooled and reused by different messages .