...
- The algorithm then splits the set into subsets based on the values of the selected attribute, and repeats the process for each subset until one of the following conditions is met:
  - The subset is pure, meaning all the instances belong to the same class.
  - There are no more unused attributes.
  - There are no more instances in the subset.
- The algorithm then assigns a class label to each leaf node based on the majority class of the instances in the subset.
- The algorithm can be used to classify new test cases by traversing the decision tree using the features of the test case to arrive at a leaf node.
- The algorithm has some limitations, such as :
  - It does not guarantee an optimal solution, as it can get stuck in local optima.
  - It can overfit to the training data, meaning it may not generalize well to unseen data. To avoid overfitting, smaller decision trees should be preferred over larger ones, and pruning techniques can be applied to reduce the size of the tree.
