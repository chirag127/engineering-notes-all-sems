...
- Initialize the parameters of the model, usually randomly or using a heuristic method.
- Repeat until convergence:
  - E-step: Estimate the missing or latent variables using the current parameters.
  - M-step: Update the parameters using the current estimates of the latent variables.

The EM algorithm is also widely used in medical image reconstruction, especially in positron emission tomography, single-photon emission computed tomography, and x-ray computed tomography.
...
- It can be slow to converge or get stuck in local optima.
- It can be sensitive to the initialization of the parameters.
- It can be difficult to choose the appropriate model structure or complexity.