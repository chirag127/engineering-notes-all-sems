...
- To find the candidate keys of a relation, one can use the following steps:
  ...
  - Identify all the combinations of attributes that are unique together, and mark them as candidate keys.
  - Eliminate any candidate keys that have redundant attributes, i.e., attributes that can be derived from other attributes in the same candidate key.
