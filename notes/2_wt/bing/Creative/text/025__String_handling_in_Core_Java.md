...
- The `String` class provides many methods for creating, comparing, modifying, and searching strings.
- Some of the common methods of the `String` class are:
  ...
  - `indexOf(int ch, int fromIndex)` : returns the index of the first occurrence of the specified character in the string, starting from the specified index, or -1 if the character is not found.
  - `indexOf(String str)` : returns the index of the first occurrence of the specified substring in the string, or -1 if the substring is not found.
  - `indexOf(String str, int fromIndex)` : returns the index of the first occurrence of the specified substring in the string, starting from the specified index, or -1 if the substring is not found.
  - `lastIndexOf(int ch)` : returns the index of the last occurrence of the specified character in the string, or -1 if the character is not found.
  - `lastIndexOf(int ch, int fromIndex)` : returns the index of the last occurrence of the specified character in the string, starting backward from the specified index, or -1 if the character is not found.
  - `lastIndexOf(String str)` : returns the index of the last occurrence of the specified substring in the string, or -1 if the substring is not found.
  - `lastIndexOf(String str, int fromIndex)` : returns the index of the last occurrence of the specified substring in the string, starting backward from the specified index, or -1 if the substring is not found.
  ...
  - `valueOf(char c)` : returns the string representation of the specified character.
  - `valueOf(char[] data)` : returns the string representation of the specified character array.
  - `valueOf(char[] data, int offset, int count)` : returns the string representation of a subarray of the specified character array, starting from the specified offset and containing the specified number of characters.
  ...
  - `valueOf(int i)` : returns the string representation of the specified integer value, using the decimal radix.
  - `valueOf(long l)` : returns the string representation of the specified long value, using the decimal radix.

- In addition to the `String` class, Java also provides two other classes for string manipulation: `StringBuilder` and