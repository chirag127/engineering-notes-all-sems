...
A vector space is a set of vectors that can be added together and multiplied by scalars to produce another vector in the same set. The following are the properties of a vector space:
...
9. Associativity of scalar multiplication: If c and d are scalars and v is a vector in the vector space, then (cd)v = c(dv).
10. Multiplicative identity: There exists a scalar 1 such that for any vector v in the vector space, 1v = v.

These properties define a vector space and must hold for any set of vectors to be considered a vector space. Vector spaces are fundamental in the study of linear algebra and have applications in many fields, including AI, ML, and data science.