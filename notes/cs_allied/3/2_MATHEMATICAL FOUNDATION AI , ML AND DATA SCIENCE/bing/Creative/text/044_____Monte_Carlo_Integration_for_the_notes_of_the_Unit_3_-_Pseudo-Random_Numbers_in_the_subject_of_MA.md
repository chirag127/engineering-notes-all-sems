...
- The disadvantage of Monte Carlo integration is that it is a non-deterministic approach, meaning that each realization provides a different outcome, and the error of the approximation depends on the variance of the integrand .
- The error of the Monte Carlo estimator can be reduced by using more random points, or by using variance reduction techniques, such as importance sampling, stratified sampling, or quasi-Monte Carlo methods  .