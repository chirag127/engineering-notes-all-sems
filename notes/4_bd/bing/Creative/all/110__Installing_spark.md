...
- To stop Spark, you can use the Ctrl+C keyboard shortcut or the exit() or quit() command in the shell. You can also use the stop() method of the SparkContext object in your application code. For example, you can use the following command on Linux or Mac OS to stop the spark-shell:

```bash
exit()
```

- To uninstall Spark, you can simply delete the installation directory and remove the environment variables. For example, you can use the following commands on Linux or Mac OS to uninstall Spark:

```bash
rm -rf /opt/spark/spark-3.2.0-bin-hadoop3.2
unset SPARK_HOME
unset PATH
